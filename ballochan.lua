...
Ballochan:include(HasPhysics);


function Ballochan:initialize(ctx)

	
    self.ctx = ctx -- wsl: note change - now has ctx as member
--    wendyprint("Ballochan:initialise(ctx): self.ctx is: ")
--    wendyprint(self.ctx)
--    self.ctx.physics:newBody(self)

    wendyprint("Ballochan:initialise(ctx): self.body is: ")
    wendyprint(self.body)
	if self.ctx.restart_physics then
--		self.x = 400
--		self.y = 250
--		self.a = 0	
		wendyprint("Ballochan:initialise(ctx): creating body! ")
...
--	local new = Ballochan.create() --wsl: note change noe use new not create,and now pass in ctx.
    ctx.saved_physics_body = self.body
	local new = Ballochan:new(ctx)
	...
	if use_mouse then 
	    self:fly() --wsl
	    print("Ballochan:newState: Using Mouse!")
--		new.x = 2*(ctx.mouse.x*0.01+oldstate.x*0.99)
--		new.y = 2*(ctx.mouse.y*0.01+oldstate.y*0.99)
	else 
		print("Ballochan:newState: Using Physics Only!")
--        wendyprint("In Ballochan:newState: Using Physics Only...self is:")
--	    wendyprint(self)
--	    print_table(self)
	    wendyprint("In Ballochan:newState: Using Physics Only...self.body is:")
--	    wendyprint(self.body)
...
-- wsl: ballochan is a faceless chan...
--	table.insert(d, {
